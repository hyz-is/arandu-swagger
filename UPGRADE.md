...
`v0.42.0` and Hesape to `v0.21.0` or newer before taking this release; an
application below either floor will not build against it.

```bash
go get github.com/arandu-io/framework@v0.42.1
go get github.com/arandu-io/hesape@v0.21.1
go get github.com/hyz-is/arandu-swagger
```

`arandu.mod.toml` now declares `framework = ">= 0.42"`.
